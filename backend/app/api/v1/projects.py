...
def report_project(project_id: UUID, db: Session, current_user: User):
    ...
    require_project_permission(
        ...
        "projects.read", "projects.manage", "projects.manage_assigned",
        "findings.read", "findings.manage", "security.reports.read", "security.reports.manage",
    )
    return project
...
def get_project_access(
    ...
    if project.owner_id != current_user.id:
        raise HTTPException(403, "Only the project owner can manage access")
    return {
        "users": db.query(User).filter(User.id != current_user.id, User.is_active.is_(True), User.organization_id == current_user.organization_id).order_by(User.email).all(),
...
def get_project(
    ...
    require_project_permission(db, current_user, project, "projects.read", "projects.manage", "projects.manage_assigned")
...
def update_project(
    ...
    require_project_permission(db, current_user, project, "projects.manage", "projects.manage_assigned")
...
def get_project_scans(
    ...
    require_project_permission(db, current_user, project, "scans.read", "scans.manage", "projects.read", "projects.manage", "projects.manage_assigned")
...
def get_project_details(
    ...
    require_project_permission(db, current_user, project, "projects.read", "projects.manage", "projects.manage_assigned", "findings.read", "scans.read")
