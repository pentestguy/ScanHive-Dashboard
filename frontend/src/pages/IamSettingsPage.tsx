...
export function IamSettingsPage() {
  ...
  const canManageIam = profileQuery.data?.effective_permissions.some(
    (permission) => permission.startsWith("iam.") && permission.endsWith(".manage"),
  ) ?? false;
  ...
  const roleColumns = [
    ...
    {
      title: "Permissions", key: "permissions", render: (_: unknown, role: IamRole) => (
        <div className="iam-permission-list">
          {role.permissions.map((permission) => <span key={permission}>{permission.replaceAll(".", " · ")}</span>)}
        </div>
      ),
    },
    { title: "Type", key: "type", render: (_: unknown, role: IamRole) => <span className="iam-role-type">{role.is_system ? "System" : "Custom"}</span> },
    {
      title: "", key: "actions", render: (_: unknown, role: IamRole) => (
        <div className="iam-role-actions">
          ...
          <button
            ...
            disabled={role.is_system || roleDeleteMutation.isPending}
