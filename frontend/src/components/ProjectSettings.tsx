import { App, AutoComplete, Button, Card, Form, Input, Menu, Popconfirm, Table, Tag, Typography } from "antd";
      ...
      {section === "general" ? (
        <Card className="project-settings-ant-card">
          <Form form={form} layout="vertical" requiredMark="optional" onFinish={(values) => generalMutation.mutate(values)}>
            ...
            <Form.Item className="settings-form-actions"><Button type="primary" htmlType="submit" loading={generalMutation.isPending}>Save changes</Button></Form.Item>
          </Form>
        </Card>
      ) : (
        <Card className="project-settings-ant-card">
          ...
          />
          <div className="settings-form-actions"><Button type="primary" loading={accessMutation.isPending} onClick={() => accessMutation.mutate()}>Save access</Button></div>
        </Card>
      )}
    </div>
  );
